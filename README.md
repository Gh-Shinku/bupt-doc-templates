...
| `date` | 日期，可直接用 `today.display(...)`；留空则不显示日期行 |
...
| `abstract` | 摘要内容，默认为空；为空时不显示摘要 |
| `keywords` | 关键词数组，默认为空；为空时不显示关键词 |
| `keyword-label` | 关键词标签，默认为 `关键词` |

摘要和关键词由 `bupt-doc` 统一渲染在目录前，输出顺序为：封面 → 摘要/关键词 → 目录 → 正文。

摘要和关键词都采用“空值即关闭”的方式控制显示：

- `abstract: []`：不显示摘要标题和摘要正文。
- `keywords: ()`：不显示关键词行。
